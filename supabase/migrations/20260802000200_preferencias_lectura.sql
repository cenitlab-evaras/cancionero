...
create table if not exists public.preferencias_lectura (
...
comment on column public.preferencias_lectura.transposicion is
  'Semitonos, -11..11. RN-13: al llegar a ±12 el ciclo vuelve a 0, así que 12 no es un valor válido.';
comment on column public.preferencias_lectura.tamano_letra is
...
create index if not exists preferencias_perfil_idx
  on public.preferencias_lectura (perfil_id);

-- -----------------------------------------------------------------------------
-- RLS — clase C, predicado propio
-- -----------------------------------------------------------------------------

alter table public.preferencias_lectura enable row level security;

-- Ver: solo las propias. Sin excepción para admin ni director.
drop policy if exists preferencias_select on public.preferencias_lectura;
create policy preferencias_select on public.preferencias_lectura
...
drop policy if exists preferencias_write on public.preferencias_lectura;
create policy preferencias_write on public.preferencias_lectura
  for all using (perfil_id = auth.uid() and public.puede_ver_coro(coro_id))
