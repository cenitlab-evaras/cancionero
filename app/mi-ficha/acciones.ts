 ...
 *
 * Una ficha a medias es más útil que ninguna, y obligar a declarar el
...
export async function guardarMiFicha(raw: unknown): Promise<Resultado> {
  ...
  if (!puede(sesion.sujeto, 'editar_ficha_propia')) {
    return { ok: false, error: 'No tienes permiso para cargar tu ficha en este coro.' }
  }
  ...
  if (error) return { ok: false, error: 'No se pudo guardar tu ficha.' }
